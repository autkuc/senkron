...
async function runEndpointLoadTest(serverPort, path, method, body, headers = {}, totalRequests = 200, concurrency = 20) {
    const latencies = [];
    const statusCounts = {};
    let completed = 0;
...
async function runFullBenchmark() {
    const PORT = 4099;
    const server = server_1.app.listen(PORT, async () => {
        console.log(`\n======================================================`);
        console.log(`🚀 SENKRON BACKEND LOAD PERFORMANCE BENCHMARK SUITE`);
        console.log(`======================================================\n`);
        // 1. Benchmark Video Route Decision Endpoint
        const videoResult = await runEndpointLoadTest(PORT, '/api/video/route-decision', 'POST', { fileSizeBytes: 45000000, durationSeconds: 28, hardwareConcurrency: 8 }, {}, 500, 50);
        // 2. Benchmark Quota Status Query Endpoint
        const quotaResult = await runEndpointLoadTest(PORT, '/api/quota/status', 'GET', undefined, { 'x-user-id': 'user_load_test' }, 500, 50);
        // 3. Benchmark AI Post Generation Rate-Limiting & Auth Guard
        const aiResult = await runEndpointLoadTest(PORT, '/api/ai/generate', 'POST', { topic: 'Senkron Web Components Release', tone: 'viral' }, { 'x-user-id': 'user_load_test' }, 100, 10);
        console.table([
            {
