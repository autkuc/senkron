...
export interface VideoEditorEvents {
  'senkron:ready': CustomEvent<{ duration: number }>;
  'senkron:timeupdate': CustomEvent<{ currentTime: number }>;
  'senkron:export-progress': CustomEvent<ExportProgressDetail>;
  'senkron:export-complete': CustomEvent<{ outputBlobUrl: string; duration: number }>;
  'senkron:video-attached': CustomEvent<VideoAttachedDetail>;
  'senkron:modal-close': CustomEvent<void>;
  'senkron:error': CustomEvent<{ message: string; error?: unknown }>;
}
