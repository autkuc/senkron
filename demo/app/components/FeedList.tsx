...
export const FeedList: React.FC<FeedListProps> = ({
  ...
  return (
    <div className="space-y-3">
      {/* Feed Tabs */}
      <div className="flex border-b border-slate-800/80 bg-[#090d16] sticky top-12 z-10">
        {[
          { id: 'flow', label: 'Akış' },
          ...
          >
            <span>{tab.label}</span>
            {activeTab === tab.id && (
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-sky-500"></div>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {posts.map((post) => (
          <article
            key={post.id}
            className="p-4 rounded-xl bg-[#0f1624] border border-slate-800/80 hover:border-slate-700/60 transition-colors space-y-3"
          >
            {/* Post Header */}
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div
                  className="w-9 h-9 rounded-full flex items-center justify-center font-semibold text-xs text-white"
                ...
                <div>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-slate-500 text-xs font-mono">{post.authorHandle}</span>
                    <span className="text-slate-600 text-xs">• {post.timeAgo}</span>
                  </div>
                </div>
              </div>

              <button className="text-slate-500 hover:text-slate-300 text-xs p-1">
            ...
            {/* Post Content */}
            <div className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap pl-0.5">
              {post.content}
            </div>
